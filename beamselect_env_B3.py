...
import numpy as np
from gym import Env
from gym.spaces import Box, MultiDiscrete, Discrete, Dict, Tuple
...
class BeamSelectionEnv(Env):
    ...
    def reset(self):
        ...
        self._state = {'chosen_ue': np.array(0), 'ch_mag': np.array([0]).astype(np.float),'pkt_trx':np.array([0])}
        #print(self._state)
        return self._state
